...
def _prepare_colorarray(arr, force_copy=False, *, channel_axis=-1):
    ...
    return _func(arr, force_copy=force_copy) # mengkonversi nilai 0-255 menjadi 0-1
...
def rgb2hsv(rgb, *, channel_axis=-1):
    ...
    """
    input_is_one_pixel = rgb.ndim == 1 # mengecek dimensi gambar
    if input_is_one_pixel:
        rgb = rgb[np.newaxis, ...] # menambahkan 1 dimensi

    arr = _prepare_colorarray(rgb, channel_axis=-1) # konversi 0-255 menjadi range 0-1 
    out = np.empty_like(arr)

    # -- V channel
    out_v = arr.max(-1) #ambil nilai maksimum tiap dimensi, makanya jadi 2D array

    # -- S channel
    delta = arr.ptp(-1) # ambil (max-min) tiap dimensi
    ...
    # -- H channel
    # red is max
    idx = (arr[..., 0] == out_v) # cek index yang memiliki nilai red max
    out[idx, 0] = (arr[idx, 1] - arr[idx, 2]) / delta[idx] # jika nilai red max maka atur derajat pada index yang nilai red-nya max

    # green is max
    idx = (arr[..., 1] == out_v)# cek index yang memiliki nilai green max
    out[idx, 0] = 2. + (arr[idx, 2] - arr[idx, 0]) / delta[idx]# jika nilai green max maka atur derajat pada index yang nilai green-nya max

    # blue is max
    idx = (arr[..., 2] == out_v)# cek index yang memiliki nilai blue max
    out[idx, 0] = 4. + (arr[idx, 0] - arr[idx, 1]) / delta[idx]# jika nilai blue max maka atur derajat pada index yang nilai blue-nya max
    out_h = (out[..., 0] / 6.) % 1.
    out_h[delta == 0.] = 0.
